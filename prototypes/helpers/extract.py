...
sys.path.append("C:\\Users\\plank\\Documents\\git\\r4r-prototypes\\prototypes\\helpers")
...
def extract_place(place="Weißenborn, Sachsen",
    ...
    if not Path(file_input).is_file():
        raise FileNotFoundError(Path(file_input).absolute())
...
def assign_pop(graph, city_csv, grid_pop_file):
    ...
    nearest_nodes_to_middlepoints = ox.nearest_nodes(graph, y, x)
